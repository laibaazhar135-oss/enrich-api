...
Kill switch: LLM_ENABLED=false makes the endpoint skip the model entirely and return a clean 503 with a message. This exists so someone who is not you can turn the feature off during an outage or bill spike without deploying code.
...
LLM_BASE_URL	http://localhost:11434/v1	Provider base URL. Swap to https://openrouter.ai/api/v1 in one line.
LLM_MODEL	llama3.2	Model ID. Swap to openrouter/free for hosted.
LLM_API_KEY	ollama	API key. For Ollama this is required but ignored. For OpenRouter, use your real key.
LLM_ENABLED	true	Kill switch. Set to false to disable model calls instantly.
LLM_STUB	0	Stub mode. Set to 1 to return fake schema-valid data without calling the model.
PORT	3000	HTTP port for the Express server.
Swapping providers is three env vars. That is the entire point of using the openai SDK against an OpenAI-compatible endpoint.
Cost Analysis
Since this build uses Ollama (local), the per-call cost is $0.00.
Sample cost log entry (logs/cost.jsonl)
JSON
{
  "TimeStamp": "2026-08-31T17:44:49.004Z",
  "PromptVersion": "v1",
  "ModelName": "llama3.2",
  "InputTokens": 420,
  "OutputTokens": 85,
...
Estimated cost at 10,000 requests/day (if switched to OpenRouter)
Using openrouter/free tier: $0.00 (free tier is 50 requests/day; beyond that you would need a paid model).
If switched to a paid model at ~$0.70/million input tokens and ~$0.90/million output tokens:
...
Structured output: Try using the model's native response_format JSON mode (if supported by the provider) so malformed JSON becomes impossible rather than merely unlikely.
...
Provider-	Ollama (local) / OpenRouter (swap in 3 env vars)
Model	-llama3.2